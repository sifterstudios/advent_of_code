...
func main() {
	...
	for i := 0; i < len(seeds); i++ {
		seed := seeds[i]
		soil := getNextPosition(seed, seedToSoilMap)
		...
		print("Added entry to almanac: Seed: ", seed, " Soil: ", soil, " Fertilizer: ", fertilizer, " Water: ", water,
...
func grabSeeds(i int, currentLine string) {
	if i == 0 {
		...
		for j := 0; j < len(seedsString); j++ {
			seed, _ := strconv.Atoi(seedsString[j])
