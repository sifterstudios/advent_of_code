...
func main() {
	file, err := os.Open("./2023/input/day7_part1_input")
	if err != nil {
	...
	determineStrengthOfHandsAndSort(allHandBets, fiveOfAKindIndexes, true)
	determineStrengthOfHandsAndSort(allHandBets, fourOfAKindIndexes, false)
	determineStrengthOfHandsAndSort(allHandBets, fullHouseIndexes, false)
...
func determineStrengthOfHandsAndSort(allHandBets []handBet, indexes []int, isFiveOfAKind bool) {
	localIndexes := indexes
	highestRank := -1
	highestRankIndex := -1
	if isFiveOfAKind {
		for len(localIndexes) > 0 {
			for i := 0; i < len(localIndexes); i++ {
				cardString := allHandBets[localIndexes[i]].allCards
				if cardString[i] > highestRank {
					highestRank = cardString[i]
					highestRankIndex = localIndexes[i]
				}
				finalRanks = append(finalRanks, allHandBets[highestRankIndex])
				localIndexes = removeElement(localIndexes, highestRankIndex)
			}
	...
	bubbleSortHandsAndAddToFinalRanks(allHandBets, indexes)
}
...
func bubbleSortHandsAndAddToFinalRanks(allHandBets []handBet, indexes []int) {
...
func determineHandRank(handBet handBet, idx int) {
	...
	} else if len(distinctNumbers) == 1 {
		fiveOfAKindIndexes = append(fiveOfAKindIndexes, idx)
	} else {
		println("Could not find distinct numbers in hand")
	}
}
